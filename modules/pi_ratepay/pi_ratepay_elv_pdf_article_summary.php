 ...
 */
class pi_ratepay_elv_pdf_article_summary extends pi_ratepay_elv_pdf_article_summary_parent
{
    ...
    protected function _setPaymentMethodInfo(&$iStartPos)
    {
        ...
        if ($oPayment->oxpayments__oxid->value == "pi_ratepay_elv") {
            $oPayment = oxNew('oxpayment');
            $oPayment->loadInLang($this->_oData->getSelectedLang(), $this->_oData->oxorder__oxpaymenttype->value);

            $text = $this->_oData->translate('PI_RATEPAY_ELV_PDF_PAYMETHOD') . " " . $oPayment->oxpayments__oxdesc->value;
            $this->font('Arial', '', 10);
    ...
    protected function _setPayUntilInfo(&$iStartPos)
    {
        ...
        $oPayment->loadInLang($this->_oData->getSelectedLang(), $this->_oData->oxorder__oxpaymenttype->value);
        if ($oPayment->oxpayments__oxid->value == "pi_ratepay_elv") {
            $paymentMethod = pi_ratepay_util_utilities::getPaymentMethod($oPayment->oxpayments__oxid->value);
